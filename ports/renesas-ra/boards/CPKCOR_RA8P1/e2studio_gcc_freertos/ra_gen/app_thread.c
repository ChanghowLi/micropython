...
static StaticTask_t app_thread_memory;
                ...
                static uint8_t app_thread_stack[1024] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
...
const rm_freertos_port_parameters_t app_thread_parameters = { .p_context = (void*)NULL, };
...
void app_thread_create(void)
{
	...
	app_thread = xTaskCreateStatic(
                    ...
                    #endif
			app_thread_func, (const char*)"app_thread", 1024 / 4, // In words, not bytes
