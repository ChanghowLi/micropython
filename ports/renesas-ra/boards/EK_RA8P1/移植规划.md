...
**目标**：在 EK-RA8P1 板子上通过串口看到 MicroPython REPL 提示符（`>>>`）。
...
**替代方案**：将 RA8P1 所需的 FSP 文件（最新版本）全部放在 `boards/EK_RA8P1/fsp/` 目录中。Makefile 通过 `mpconfigboard.mk` 传入的变量来判断是否为 EK_RA8P1 构建，如果是，则将 `HAL_DIR`、`CMSIS_DIR`、`STARTUP_FILE`、`SYSTEM_FILE` 重定向到板子目录下的 FSP 副本，**完全旁路 `lib/fsp`**。

```
构建 EK_RA6M2：                  构建 EK_RA8P1：
    lib/fsp  (v4.4.0)                lib/fsp  (v4.4.0, 不使用)
    lib/cmsis/inc (现有)             boards/EK_RA8P1/fsp/  (最新 FSP)
                                     boards/EK_RA8P1/cmsis/ (可选)
```
...
### [1] 在 boards/EK_RA8P1/fsp/ 部署 FSP 文件

从最新 FSP 发行版（如 v6.4.0）中，**只提取里程碑 1 需要的文件**：

```
boards/EK_RA8P1/
├── fsp/
│   ├── README.md                              # 占位符，防止触发 git submodule update
│   ├── ra/
│   │   └── fsp/
│   │       ├── inc/                           # 公共 API 头文件
│   │       │   ├── r_ioport.h                 #   - IOPORT
│   │       │   ├── r_sci_uart.h               #   - SCI UART
│   │       │   ├── r_flash_hp.h               #   - Flash（类型待确认）
│   │       │   └── api/
│   │       │       ├── r_ioport_api.h
│   │       │       ├── r_sci_uart_api.h
│   │       │       └── r_flash_api.h
...
**注意事项**：
- `R7KA8P1KF_core0.h` 可能内部 `#include` 其他头文件（如 `core_cm85.h`、`system.h`）。需要追踪这些依赖并一并复制。
- 如果 `lib/cmsis/inc/core_cm85.h` 版本足够，可以继续使用它，省去 `cmsis/` 目录。但 FSP v6.4.0 的 CMSIS 头文件可能更新了 M85 相关定义，建议同步。
- `r_flash_hp` 是 RA6M2/RA6M5 上使用的 Flash 驱动（HP = High Performance）。RA8P1 可能也用类似的名字，也可能完全不同（如 `r_flash_m85`）。需确认。
...
这样 EK_RA8P1 使用 `FSP_README = .../boards/EK_RA8P1/fsp/README.md`，只要在 `fsp/` 下放了 `README.md` 占位文件就不会触发 `git submodule update`。
...
**重要**：这里假设 RA8P1 也用 `r_flash_hp`。如果不对，需要在 Makefile 中新增 RA8P1 专用 Flash 驱动路径。

#### 3d. AF 文件映射（第 448-462 行新增）

```makefile
ifeq ($(CMSIS_MCU),$(filter $(CMSIS_MCU),RA8P1))
AF_FILE = boards/ra8p1_af.csv
endif
```

#### 3e. gchelper_thumb2 条件（第 389-393 行）
...
RA8P1 (M85) 与 RA6M5 (M33) 同为 ARMv8.1-M，可能需要同样的显式汇编规则。将条件扩展为 `RA6M5 RA8P1`，或在编译后根据错误决定是否添加。
...
使用 **Renesas e² studio** 或 **FSP Configurator**，选择与步骤 [1] 相同版本的 FSP。

#### 里程碑 1 最简 FSP Stack 配置

| FSP Stack | 用途 |
|-----------|------|
| **Clock** | 配置系统时钟（HOCO/LOCO/SubClock/PLL），外设时钟分频 |
| **IOPORT** | GPIO 基础，所有引脚都需要通过它配置 |
| **SCI UART ×1** | REPL 串口，选 EK-RA8P1 板载 USB-UART 对应的 SCI 通道 |
| **Flash** | 内部 Flash 操作，用于固件升级和文件系统 |
| **BSP** | 自动包含：IO、IRQ、Register Protection、Delay 等 |
...
提取以下文件放到 `boards/EK_RA8P1/`：

| 文件 | 路径 |
|------|------|
| `common_data.c` / `.h` | `ra_gen/` |
| `hal_data.c` / `.h` | `ra_gen/` |
| `pin_data.c` | `ra_gen/` |
| `vector_data.c` / `.h` | `ra_gen/` |
| `bsp_clock_cfg.h` | `ra_gen/` |
| `bsp_pin_cfg.h` | `ra_gen/` |
| FSP 板级配置头文件 | `ra_cfg/fsp_cfg/bsp/` 下所有 `.h` |
| `ra8p1_ek_conf.h` | 板子根目录 |

**回到目录结构确认**：此时 `boards/EK_RA8P1/` 下应该已有：

```
EK_RA8P1/
├── 移植规划.md                    # 本文档
├── mpconfigboard.mk               # 步骤 [9]
├── mpconfigboard.h                # 步骤 [9]
├── pins.csv                       # 步骤 [10]
├── board.json                     # 步骤 [10]
├── ra8p1_ek.ld                    # 步骤 [11]
├── ra8p1_ek_conf.h                # 步骤 [8] 生成
├── fsp/                           # 步骤 [1]
├── cmsis/                         # 步骤 [1]
├── ra_gen/                        # 步骤 [8] 生成
│   ├── common_data.c / .h
...
### [9] 创建 mpconfigboard.mk 和 mpconfigboard.h（最简配置）
...
数据来源：**EK-RA8P1 原理图** + RA8P1 数据手册。需要对照板子上的排针和功能确定哪些引脚是真正引出的。
...
创建 `boards/EK_RA8P1/ra8p1_ek.ld`，参照 `boards/EK_RA6M2/ra6m2_ek.ld` 但**只定义内部存储器区域**：

需要从 RA8P1 数据手册获取以下内存布局：

| 区域 | 用途 | 基地址 | 大小 |
|------|------|--------|------|
| 内部 Flash | 固件代码 + MicroPython 内部文件系统 | 0x????_???? | ??? KB |
| 内部 SRAM | 运行时数据 + 堆 + 栈 | 0x????_???? | ??? KB |
| ITCM（如有） | 代码紧密耦合内存 | — | — |
| DTCM（如有） | 数据紧密耦合内存 | — | — |

**重要**：
- RA8P1 是双核，Flash 和 SRAM 可能在两个核之间有划分。需要确认 core0 可用的地址范围。
- Cortex-M85 的向量表默认在 `0x0000_0000`，但实际物理 Flash 可能映射到其他地址。需要确认 VTOR 配置。
- `.id_code` section 的地址需要与 FSP 配置中的 ID Code 区域一致。
...
- 使用 `R7KA8P1KF_core0.h` 而非 `core1.h`
- 在 `startup.c` 或 FSP 配置中确保 core1 保持 reset 或 halt 状态
- Flash 和 SRAM 的地址范围需要是 core0 的视角

#### 12b. TrustZone

Cortex-M85 支持 TrustZone（Security Extension）。如果 FSP 配置启用了 TrustZone：
- 中断向量表布局会变（Secure + Non-Secure 两张表）
- 内存访问有安全属性

**里程碑 1 建议**：在 FSP 配置中将 TrustZone 设为 flat（全 Non-Secure）模式，避免增加复杂度。后续里程碑再决定是否需要 TrustZone。

#### 12c. gchelper_thumb2.s
...
#### 12d. MICROPY_EMIT_THUMB / INLINE_THUMB
