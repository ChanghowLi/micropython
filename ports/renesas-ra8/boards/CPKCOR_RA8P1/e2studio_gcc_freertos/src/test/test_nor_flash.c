...
#define TEST_NOR_FLASH_EN_FULL_CHECK	1
#endif
...
static uint8_t checkFullChipNoCache(uint32_t num_sector)
{
	...
	printf("Erase chip using time: %lu ms\r\n", (uint32_t)(time_end - time_start));
#endif
...
#endif
	printf("Average write time: %lu us, speed: %.2fKB/s\r\n", (uint32_t)(time_write_sum / j), (float)(speed_sum / (float)j));
...
static void checkSpeedRead(void)
{
	...
	if (time_start != time_end) {
		...
		printf("using cycle: %lu, ", (uint32_t)(tick_end - tick_start));
		printf("using time: %lu us, ", (uint32_t)(time_end - time_start));
